...
find_instance_by_course_id(CourseID, _{instanceId: _, semester: _, year: _, courseId: CourseID, slotId: _}).

find_degree_course_by_id(DegreeId, _{ degreeId: DegreeId, courseId: _, requirementType: _ }).

select_course_id(_{ degreeId: _, courseId: Id, requirementType: _ }, Id).

% Extracts slot id from instance id json
slot_id_from_instance(_{courseId:_,instanceId:_,semester:_,slotId: ID,year:_}, ID).
...
timeslot_from_instance_id(SlotIds, _{slotId: SlotId, day: _, periodId1: _, periodId2: _, periodId3: _}, []) :-
  \+ member(SlotId, SlotIds).
timeslot_from_instance_id(SlotIds, _{slotId: SlotId, day: DayString, periodId1: PeriodOne, periodId2: PeriodTwo, periodId3: PeriodThree}, MeetingList) :-
  member(SlotId, SlotIds),
